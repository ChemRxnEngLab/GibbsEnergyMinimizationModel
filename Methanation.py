...
def dfg(T):
    """
    function for determination of Gibbs free energy of formation of a species @ T from NIST-JANAF tables [4] by a polynomic fit, range of validity: 0 - 6000 K
...
def phi_Soave(n, T, p):
    """
    function for determination of fugacity coefficients of all gaseous species @ T, p from Soave-Redlich-Kwong Equation of State [3]
    ...
    omega_i = np.array([0.224, -0.215, 0.011, 0.343,  0.048, -0.388,   0.000, 0.037]) # vector containing acentric factors of CO2, H2, CH4, H2O, CO, He, Ar and N2 in 1 [2]
    T_c_i   = np.array([304.2,  33.18, 190.6, 647.0, 134.45,    5.2,  150.86, 126.2]) # vector containing critical temperatures of CO2, H2, CH4, H2O, CO, He, Ar and N2 in K [2]
    p_c_i   = np.array([ 73.8,  13.00,  46.1, 220.6,     35,  2.274, 48.9805,  33.9]) # vector containing critical pressures of CO2, H2, CH4, H2O, CO, He, Ar and N2 in bar [2]
...
def g_T(n, T, p, type):
    """
    function for determination of the total Gibbs free energy to be minimized [1, 2]
