...
def g_T(n, T, p, type="real gas"):
    ...
    # for fug coeffs: n.shape = (n_phases, n_species)
    ...
    z_mix = (n_G + n_L) / np.sum(n_G + n_L) # array containing molar fractions of all species
    ...
    if type == 'ideal gas':
        ...
        phii = phi_Soave(z_mix, T, p * 1e5) # function needs pressure in Pa and molar fractions in gas phase
    else:
    ...
    res = np.dot(n_G, dfgi) + R * T * (np.dot(n_G, np.log(phii[:6] * p * y_gas / p0))) + np.dot(n_L, dfgi) + R * T * (np.dot(n_L, np.log(phii[6:] * p * x_liq / p0)))
...
def calc_bounds(x0):
    ...
    n_i_t  = n_G + n_L                                                                                              # total initial molar amount in mol 
    max_C   = n_i_t[0] + n_i_t[3] + n_i_t[4] #+ n0[0] + n0[3] + n0[4]                                                      # molar amount of carbon in the system in mol
    max_H   = 2 * n_i_t[1] + 2 * n_i_t[2] + 4 * n_i_t[4] #+ 2 * n0[1, 1] + 2 * n0[1, 2] + 4 * n0[1, 4]               # molar amount of hydrogen in the system in mol
    max_O   = 2 * n_i_t[0] + n_i_t[2] + n_i_t[3] + n_i_t[4] #+ 2 * n0[1, 0] + n0[1, 2] + n0[1, 3] + n0[1, 4]         # molar amount of oxygen in the system in mol
    max_N   = 2 * n_i_t[5] #+ 2 * n_i_t[1, 5]                                                                           # molar amount of nitrogen in the system
    ...
    bnds = (
        ...
        (0, max_CO2), (0, max_H2), (0, max_H2O), (0, max_CO), (0, max_MeOH), (0, max_N2)  # bounds for liquid phase
        )
...
def calc_eq(T,p,x0,type='real gas'): # removed guess
    ...
    cons = [{'type': 'eq', 'fun': element_balance, 'args': [n0]},
            # set each molar amount greater or equal to zero
            {'type': 'ineq', 'fun': lambda n: n},
            # sum of molar amounts in each phase must be greater or equal to zero
            {'type': 'ineq', 'fun': lambda n: n[:6]},
            {'type': 'ineq', 'fun': lambda n: n[6:]}]


    # maybe second constraint could be added that sum of elements in both phases must equal the sum of elements in the inlet composition
    ...
    rndm_guesses_L = np.array([1e-20, 1e-20, 1e-20, 1e-20, 1e-20, 1e-20])

    print(rndm_guesses_G)
    print(rndm_guesses_L)
    rndm_guesses = np.hstack((rndm_guesses_G, rndm_guesses_L))

    print(rndm_guesses)

    guesses = np.zeros((rndm_no + 3, len(x0)))
    ...
    for i, guess in enumerate(guesses):
        ...
        sol = minimize(g_T, guess, args=(T, p, type), method='SLSQP', constraints = cons, bounds=bnds, options = {'disp': False, 'maxiter': 10000, 'ftol': 1e-5})
        # Define the local optimization method
        '''local_minimizer = {
            'method': 'SLSQP',
            'constraints': cons,
            'bounds': bnds,
            'options': {'disp': False, 'maxiter': 10000, 'ftol': 1e-5}
        }
        ...
        sol = basinhopping(
            func=g_T,
            x0=guess,
            minimizer_kwargs={'args': (T, p, type), **local_minimizer},
            niter=100,  # Number of basin-hopping iterations (you can adjust this)
            T=1.0,  # Temperature parameter that controls the stochastic jumps
            stepsize=0.5,  # Step size for random perturbations (tune based on your problem)
            disp=True  # Set to False if you don't want output
        )'''
