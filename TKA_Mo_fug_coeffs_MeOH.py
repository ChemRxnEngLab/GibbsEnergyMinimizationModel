...
def phi_Soave_2(z, T, p):
    ...
    # Parameter         # CO2   # H2     # H2O   # CO  # MeOH # N2
    omega_i = np.array([ 0.225, -0.217,  0.343,  0.045, 0.565, 0.037])      # array containing acentric factors in 1 (Perry's)
    T_c_i   = np.array([304.12,  32.98, 647.13, 132.85, 512.64, 126.2])      # array contaning critical temperatures in K (Perry's)
    p_c_i   = np.array([  73.74, 12.93,  219.4,   34.94, 80.97,   33.9]) * 1e5 # array containing critical pressures in Pa (Perry's)
    # Green, Don W.; Perry, Robert H. (2003): Perry's chemical engineers' handbook. 7th ed., internat. ed., [Nachdr.]. New York: McGraw-Hill.
    # params of liq
...
def phi_Soave_3(z, T, p):
    ...
    z_i_G   = z[:6] # array of gas phase molar fractions (CO2, H2, H2O, CO, MeOH and N2) in 1
    z_i_L   = z[6:] # array of liquid phase molar fractions (MeOH and N2) in 1
    ...
    ratio_a_liq = (alpha_i**0.5 * T_c_i / p_c_i**0.5) / (np.dot(z_i_L, (alpha_i**0.5 * T_c_i / p_c_i**0.5))) # array containing ratios sqrt(a_i/a) in 1
    ratio_b_liq = (T_c_i / p_c_i) / (np.dot(z_i_L, (T_c_i / p_c_i)))                                         # array containing ratios (b_i/b) in 1

    A_L = 0.42747 * p / T**2 * (np.dot(z_i_L, (T_c_i * alpha_i**0.5 / p_c_i**0.5)))**2 # parameter A
    B_L = 0.08664 * p / T * np.dot(z_i_L, (T_c_i / p_c_i))                             # parameter B
    ...
    # use np.roots
    Z_solve = ces.solve(1, -1, A_G - B_G - B_G**2, -A_G * B_G)
    Z_solve_liq = ces.solve(1, -1, A_L - B_L - B_L**2, -A_L * B_L)

    Z_solve = Z_solve[np.isreal(Z_solve)]
    Z_solve_liq = Z_solve_liq[np.isreal(Z_solve_liq)]

    Z_solve= np.real(Z_solve)
    Z_solve_liq = np.real(Z_solve_liq)

    VLE = False

    if len(Z_solve) == 3 or len(Z_solve_liq) == 3:
        VLE = True

    Z_liq = np.min(Z_solve_liq)
    Z_vap = np.max(Z_solve)

    res_vap = np.exp(ratio_b * (Z_vap - 1) - np.log(Z_vap - B_G) - A_G / B_G * (2 * ratio_a - ratio_b) * np.log(1 + B_G / Z_vap))             # array of fugacity coefficients in 1
    res_liq = np.exp(ratio_b_liq * (Z_liq - 1) - np.log(Z_liq - B_L) - A_L / B_L * (2 * ratio_a_liq - ratio_b_liq) * np.log(1 + B_L / Z_liq)) # array of fugacity coefficients in 1

    return res_liq, res_vap, VLE
